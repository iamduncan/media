...
class TransferBehavior extends ModelBehavior {
    ...
    public function transferMeta($Model, $resource) {
        ...
        if (TransferValidation::url($resource, array('scheme' => 'http'))) {
            $resource = array_merge(
                    $defaultResource, pathinfo(parse_url($resource, PHP_URL_PATH)), array(
            ...
            $resource = array_merge(
                    $defaultResource, pathinfo($resource), array(
                'file' => $resource,
                'host' => 'localhost',
            ...
            $resource = array_merge(
                    $defaultResource, pathinfo($resource['name']), array(
                'file' => $resource['name'],
    ...
    protected function _source($Model, $resource) {
        if (TransferValidation::fileUpload($resource)) {
            return array_merge(
                    $this->transferMeta($Model, $resource), array('error' => $resource['error'])
            );
    ...
    protected function _temporary($Model, $resource) {
        if (TransferValidation::fileUpload($resource) && TransferValidation::uploadedFile($resource['tmp_name'])) {
            return array_merge(
                    $this->transferMeta($Model, $resource['tmp_name']), array('error' => $resource['error'])
            );
        } elseif (MediaValidation::file($resource)) {
            return $this->transferMeta($Model, $resource);
